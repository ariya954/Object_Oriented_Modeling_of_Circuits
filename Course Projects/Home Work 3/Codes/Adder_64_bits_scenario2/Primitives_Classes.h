...
//	bus(const int i) { v = bitset <sizeof(int)*8> (i).to_string(); }
//	bus(const bus& a) { v = a.v; } // Copy constructor for =
	...
	void fill_given_range(int begin_index, int end_index, string s)
	{
		int end = v.length() - 1 - begin_index;
		int begin = v.length() - 1 - end_index;
		for(int i = begin; i <= end; i++)
			v.at(i) = s.at(i - begin);
	}
	
	string get_bus_values()
	{
		return v;
	}
	...
	void resize(int i, char c) { v.resize(i, c); }
};
class Mux {
	bus *i1, *i2, *i3, *o1;
public:
